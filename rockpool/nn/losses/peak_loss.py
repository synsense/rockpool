...
class PeakLoss(_Loss):
    ...
    def log_params(self, parser) -> None:
        params = [i for i in inspect.getmembers(self) if not i[0].startswith('_') and not inspect.ismethod(i[1])]
    ...
    def loss_target(self, prediction: Tensor, target: Tensor) -> Tensor:
        ...
        prediction_max_target = prediction_target[batch_indices, max_indices] / self.max_interval
        ...
        return self.mse(prediction_max_target, torch.ones_like(prediction_max_target) * self.target_output)
    ...
    def loss_nontarget(self, prediction: Tensor, target: Tensor) -> Tensor:
        ...
        for i in range(1, n_classes):
            prediction_nontarget = prediction[batch_indices, :, (target + i) % n_classes]
            loss += self.mse(prediction_nontarget, torch.zeros_like(prediction_nontarget))
...
class BinaryPeakLoss(_Loss):
    ...
    def loss_positives(self, prediction: Tensor, target: Tensor) -> Tensor:
        ...
        if sum(target == 1) > 0:
            ...
            batch_indices_positives = torch.arange(prediction_positives.size()[0]).long()
            max_indices_start = prediction_positives.max(1)[1]
            max_indices = torch.clone(max_indices_start)

            prediction_max_positives = prediction_positives[batch_indices_positives, max_indices] / self.max_interval
            for i in range(1, self.max_interval):
                max_indices = torch.clip(max_indices + 1, max=nr_time_steps)
                # when we reach the end of the array we jump back to the first max_index
                max_indices[max_indices == nr_time_steps] = max_indices_start[
            ...
            # target readout neuron should be active
            return self.mse(prediction_max_positives, torch.ones_like(prediction_max_positives) * self.target_output)
        else:
            return 0
    ...
    def loss_negatives(self, prediction: Tensor, target: Tensor) -> Tensor:
        ...
        if sum(target == 0) > 0:
            ...
            return self.mse(prediction_negatives, torch.zeros_like(prediction_negatives))
        else:
            return 0
