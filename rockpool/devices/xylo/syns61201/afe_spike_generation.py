...
try:
    ...
    try:
        ...
        debug(f"C++ spike generation library was not successful: {e}. Falling back on Jax or Python version for spike generation.")
...
if __CPP_VERSION__:
    ...
    debug(f"__CPP_VERSION: {__CPP_VERSION__}: Embedding C++ version in Python.")
    ...
    # - Try to use a Jax-accelerated version
    try:
        import jax
        ...
        if __DEBUG_MODE__:
            enable_jax = False
            raise NotImplementedError("Debug mode -- don't use Jax.")
        else:
            enable_jax = True

        info("Jax was detected. Spike generation will be done by the JIT compiled version.")
        ...
        debug(f"No C++ version, no Jax version: Using Python native spike encoding.")
