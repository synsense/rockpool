...
#
# finalized and tested on 17.09.2021
# -----------------------------------------------------------
...
class DivisiveNormalisation(Module):
    ...
    In this new DN block, we avoid using LFSRs and feed
    the input signal directly to local clock generator
    ...
    def evolve(
        ...
        for ch in range(self.size_in):
            ...
            input_copy=input_copy.repeat(self.p_local/2) # (p_local/2*num_frames*cycles_per_frame) * 1

            # and reshape the pulses into frames of size p_local*cycles_per_frame
            # S_local --> local spike generator
            S_local=input_copy.reshape(-1,self.p_local*cycles_per_frame)  # num_frames * (p_local*cycles_per_frame)
            ...
            output_spike_channels[ch] = ch * np.ones(len(output_spike_times[ch]), dtype='int')
