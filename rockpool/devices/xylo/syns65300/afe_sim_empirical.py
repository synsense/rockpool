...
"""
EMPIRICAL_CENTER_FREQS = np.asarray([40, 54, 77, 137, 203, 290, 428, 674, 1177, 1700, 2226, 3418, 5154, 7884, 11630, 16940])
EMPIRICAL_Q = 4
...
try:
    ...
    def _encode_spikes(
        ...
        Args:
            ...
            max_output (float): maximum amplitude of the signal in integrator (note that this can go upto VCC).
...
class AFESimEmprirical(Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        max_spike_per_period (int): maximum number of spikes in each rastering period. This is equal to 15 spikes in Xylo-A2.
        add_noise: bool
        ...
        self.max_spike_per_period = max_spike_per_period
        ...
        self.Q_MIS_MATCH: P_float = Parameter(10e-2)  # +/-10% random
        ...
        # this is set to be I=2e-8 for a max voltage of V=60mv
        self.V2I_GIAN: P_float = Parameter(0.333e-6)
    ...
    def _initialize_afesim(self):
        ...
        self.bpf_fc_shift : P_array = Parameter(self.BPF_FC_SHIFT * (2 * np.random.rand(1).item() - 1.0)) if self.add_mismatch else Parameter(np.zeros(self.size_out))
