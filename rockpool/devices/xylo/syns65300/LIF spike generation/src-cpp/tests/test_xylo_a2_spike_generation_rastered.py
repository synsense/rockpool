...
def rel_error_levy_global(v1, v2):
    ...
    return np.max(np.abs(w1.cumsum() - w2.cumsum()))/np.min([norm(w1,1), norm(w2,1)])
...
def test_spike_generation():
    ...
    # raster the spikes
    spikes_raster_py = TSEvent.from_raster(spikes_py, dt=dt).raster(dt=raster_dt, add_events=True)
    ...
    for channel in range(num_channels):
        ...
        rel_distance = rel_error_levy_local(spikes_raster_cpp[:,channel], spikes_raster_py[:,channel])
        ...
        plt.plot(time_vec, spikes_raster_cpp[:,channel] - spikes_raster_py[:,channel], '.')
    ...
    print(f"maximum distortion in spikes of various channels: {np.max(rel_distance_spike_channel)}")
    print(f"duration of a single run: C++ => {duration_cpp}, Python [1st run]=> {duration_py_first}, Python [2nd run] => {duration_py}")
    print("+=" * 50)
