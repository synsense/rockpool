...
class PDMADC(ModSequential):
    ...
    def __init__(self) -> None:
        ...
        self.sdm_order: int = SimulationParameter(DELTA_SIGMA_ORDER)
        """order of deltasigma modulator. Defaults to DELTA_SIGMA_ORDER."""

        self.sdm_OSR: int = SimulationParameter(PDM_FILTER_DECIMATION_FACTOR)
        """
        ...
        self.fs: int = SimulationParameter(PDM_SAMPLING_RATE)
        """sampling rate/clock rate of PDM module. Defaults to PDM_SAMPLING_RATE."""

        self.cutoff: float = SimulationParameter(AUDIO_CUTOFF_FREQUENCY)
