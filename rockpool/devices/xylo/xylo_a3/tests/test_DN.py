...
def test_DN():
    ...
    num_channels = 4
    sig_in = np.asarray(
        [i * sig_in for i in range(1, num_channels + 1)], dtype=np.int64
    ).T
...
def test_jax_vs_python_DN():
    ...
    # sinusoid signal
    sig_in = np.sin(2 * np.pi * freq * time_vec)
    sig_in = (2 ** (Q - 1) * sig_in / (np.max(np.abs(sig_in)) * (1 + EPS))).astype(
        np.int64
    )
    sig_in = np.asarray(
        [i * sig_in for i in range(1, num_channels + 1)], dtype=np.int64
    ...
    print(
        f"compilation time of jax for an input of shape {sig_in.shape}: {duration_jax_compile} sec"
    )
    ...
    print(
        f"compilation time of fjax for an input of shape {sig_in.shape}: {duration_fjax_compile} sec"
    )
    ...
    print(
        f"run time of python for an input of shape {sig_in.shape}: {duration_dn} sec\n"
    )
    ...
    for i, first in enumerate(rate_list):
        for j, second in enumerate(rate_list):
            rel_error_rate[i, j] = norm(first - second) / np.sqrt(
    ...
    for i, first in enumerate(spike_list):
        for j, second in enumerate(spike_list):
            rel_error_spike[i, j] = norm(first[:] - second[:]) / np.sqrt(
                norm(first[:]) * norm(second[:])
            )
    ...
    for k in recording_jax.keys():
        ...
        for i, first in enumerate(k_list):
            for j, second in enumerate(k_list):
                rel_error_k[i, j] = norm(first[:] - second[:]) / np.sqrt(
                    norm(first[:]) * norm(second[:])
                )
