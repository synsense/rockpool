...
def test_filterbank_chirp():
    ...
    EPS = 0.00001
    sig_in = (sig_in / (np.max(np.abs(sig_in)) * (1 + EPS)) * 2 ** (B_in - 1)).astype(
        np.int64
    )
    ...
    rel_error = np.asarray(
        [
            norm(a[:] - b[:]) / np.sqrt(norm(a[:]) * norm(b[:]))
            for (a, b) in zip(sig_out_py.T, sig_out_jax.T)
        ]
    )
    ...
    for sig_out, name in zip(sig_out_list, name_list):
        ...
        plt.title(
            f"{name}: Xylo-A3 digital filterbank output for chirp => fmin:{fmin} Hz, fmax:{fmax} Hz, sweep-time:{duration} sec"
        )
...
def test_filterbank_worstcase():
    ...
    EPS = 0.00001
    impulse = (
        impulse / (np.max(np.abs(impulse)) * (1 + EPS)) * 2 ** (B_in - 1)
    ).astype(np.int64)
    ...
    for filt_idx, imp_res_filt in enumerate(impulse_res.T):
        ...
        worst_case_input = (
            worst_case_input
            / (np.max(np.abs(worst_case_input)) * (1 + EPS))
            * 2 ** (B_in - 1)
        ).astype(np.int64)

        # use this worst case signal to trigger the filter again
        worst_case_output = fb._filter_AR(
            sig_in=worst_case_input, bd=fb.bd_list[filt_idx]
        )
    ...
    additional_num_bits_worst_case_emp = (
        np.ceil(np.log2(worst_case_amplitude))
        - (B_in - 1)
        - np.asarray([bd.B_wf for bd in fb.bd_list])
    )
...
def test_filterbank_jax_worst_case():
    ...
    for filt_idx, ar_response in enumerate(ar_out.T):
        ...
        worst_case_in = (
            ar_response / np.max(np.abs(ar_response)) / (1 + EPS) * 2 ** (Q - 1)
        ).astype(np.int64)
        ...
        worst_case_ar_amplitude_amplification.append(
            np.max(np.abs(worst_case_out)) / np.max(np.abs(worst_case_in))
        )

    # how many extra bits are needed
    worst_case_extra_bits = np.ceil(
        np.log2(worst_case_ar_amplitude_amplification)
    ).astype(np.int64)
...
def test_freq_response():
    ...
    for freq in tqdm(freq_vec):
        ...
        EPS = 0.00001
        sig_in = (
            sig_in / (np.max(np.abs(sig_in)) * (1 + EPS)) * 2 ** (B_in - 1)
        ).astype(np.int64)
