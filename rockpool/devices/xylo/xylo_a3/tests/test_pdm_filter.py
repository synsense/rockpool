...
def test_filter():
    ...
    audio_worst = TSContinuous.from_clocked(np.sign(decimation_filt.h[::-1]), dt=1/(mic.fs/mic.sdm_OSR), periodic=True)(time_vec).ravel()
    ...
    for audio in audio_list:
        ...
        audio_rec_undecimated_scaled = audio_rec_undecimated/(2**(decimation_filt.num_bits_output-1)-1)
        ...
        plt.plot(np.arange(0, len(audio_rec_undecimated_scaled))*audio_rec_undecimated_dt, audio_rec_undecimated_scaled)
        plt.plot(np.arange(0, len(audio_rec_scaled))*audio_rec_dt, audio_rec_scaled, '.-')

        plt.xlabel("time (sec)")
        plt.ylabel("PDM ADC")
        plt.grid(True)
        plt.legend(["original", "recovered-undecimated", "pdm-adc"])
        plt.title(f"Dynamic range: input signal covers a {MAX_VAL:0.8f} of the dynamic range in {decimation_filt.num_bits_output} output bits")
