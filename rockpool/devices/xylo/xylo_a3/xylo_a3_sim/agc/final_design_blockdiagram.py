...
#
# NOTE:
#*       This module contains an exact block-diagram implementation of the filter to make sure that the results are compatible
#*       with Hardware.
#
#
#
...
from alias.final_design.low_pass_design_iir_blockdiagram import DecimationFilter_IIR_BlockDiagram, FilterOverflowError
...
#===========================================================================
#*           Designed filter in block-diagram format
#===========================================================================

adc_oversampling = 2

decimation_filter = DecimationFilter_IIR_BlockDiagram(
...
audio_fs = AUDIO_SAMPLE_RATE                # around 50KHz
target_audio_passband = LOW_PASS_CORNER     # 20_000 Hz
adc_oversampling = decimation_filter.adc_oversampling_factor
decimation_filter.bd.B_in = decimation_filter.bd.B_in
...
adc = OversampledADC_IIR(
    ...
    adc_num_bits=decimation_filter.bd.B_in,
...
def sinusoid_performance():
    ...
    num_freq = 400
    min_freq = 10000
    max_freq = target_audio_passband + 2_000
    freq_vec = np.exp(np.linspace(np.log(min_freq), np.log(max_freq), num_freq))
    ...
    try:
        for freq in freq_vec:
            ...
            time_vec = np.arange(0, duration, step=1/decimation_filter.bd.fs)
            ...
            EPS = 0.0000001
            sig_in_Q = (2**(decimation_filter.bd.B_in-1) * sig_in / (np.max(np.abs(sig_in)) * (1+EPS))).astype(np.int64)
            sig_out_Q, rel_distortion = decimation_filter.evolve_full(sig_in=sig_in_Q)
            
            sig_out_Q_stable = sig_out_Q[len(sig_out_Q)//6:]
            
            max_range_covered = np.max(np.abs(sig_out_Q))/(2**(decimation_filter.bd.B_in-1))
            max_stable_range_covered = np.max(np.abs(sig_out_Q_stable))/(2**(decimation_filter.bd.B_in-1))
...
def random_performance():
    ...
    num_samples = int(decimation_filter.bd.fs * duration)
    ...
    try:
        for epoch in range(num_iter):
            ...
            sig_in_Q = (2**(decimation_filter.bd.B_in-1) * sig_out_Q / (np.max(np.abs(sig_out_Q)) * (1+EPS))).astype(np.int64)
            sig_out_Q, rel_distortion = decimation_filter.evolve_full(sig_in=sig_in_Q)
...
def worst_case_performance():
    ...
    num_periods = 1000
    duration = num_periods/audio_fs
    num_samples = int(duration * decimation_filter.bd.fs)
    
    impulse = np.zeros(num_samples)
    impulse[0] = 1
    
    impulse_Q = ((2**(decimation_filter.bd.B_in -1)-1)*impulse).astype(np.int64)
    ...
    try:
        ...
        input_AR_worst_case_Q = ( (2**(decimation_filter.bd.B_in -1)-1) * input_AR_worst_case).astype(np.int64)
    ...
    print("\nworst-case analysis for the AR part was passed!")
    print(f"maximum amplitude in the AR part encountered in worst-case analysis: {np.max(np.abs(response_worst_case_AR))}")
    print(f"number of bits devoted to AR: {decimation_filter.bd.B_w}, maximum possible amplitude in AR part: {2**(decimation_filter.bd.B_w-1)}!\n\n")
    ...
    try:
        ...
        input_MA_worst_case_Q = ( (2**(decimation_filter.bd.B_in -1)-1) * input_MA_worst_case).astype(np.int64)
    ...
    print(f"number of bits devoted to the MA part: {decimation_filter.bd.B_out}, maximum possible amplitude in MA part: {2**(decimation_filter.bd.B_out-1)-1}!\n\n")
    ...
    print(f"maximum possible amplitude in the output: {2**(decimation_filter.bd.B_in-1)}!")
...
def aliasing_performance():
    ...
    f_vec, f_response = sp.freqz(
        b=adc.decimation_filter.bd.b_taps*adc.decimation_filter.bd.surplus,
        a=adc.decimation_filter.bd.a_taps,
    ...
    plt.subplot(511)
    plt.plot(decimation_filter.bd.b_taps, '.', markersize=10, label='b_Q')
    plt.plot(decimation_filter.bd.a_taps, '.', markersize=10, label='a_Q')
    plt.legend()
    
    plt.title(f'filter taps:\nb_Q:{decimation_filter.bd.b_taps}, a_Q:{decimation_filter.bd.a_taps}, surplus:{decimation_filter.bd.surplus}\n'+\
    ...
    plt.axvline(x=target_audio_passband, linewidth=2, color='g', label=f'filter 3dB ~ {target_audio_passband/1000:0.1f} KHz')
