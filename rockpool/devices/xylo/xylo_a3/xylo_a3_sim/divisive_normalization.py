...
from rockpool.devices.xylo.xylo_a3.xylo_a3_sim.digital_filterbank import NUM_FILETRS, type_check
...
class DivisiveNormalization(Module):
    def __init__(self, num_channels: int=NUM_FILETRS, spike_rate_scale_bitshift1: int = 6, spike_rate_scale_bitshift2: int = 0, low_pass_bitshift: int=12, EPS_vec: Union[int, np.ndarray]=1,  fixed_threshold_vec: Union[int, np.ndarray]=2**(14-1 + 8 + 6), fs: int=AUDIO_SAMPLING_RATE):
        ...
        if isinstance(spike_rate_scale_bitshift1, Number):
            if not isinstance(spike_rate_scale_bitshift1, int):
                raise ValueError("number of bit-shifts used for rate scaling should be an integer!")
                
            # the same EPS for all the channels
            self.spike_rate_scale_bitshift1: P_array = Parameter( (spike_rate_scale_bitshift1 * np.ones(self.num_channels)).astype(np.int64) )
            
        else:
            if len(spike_rate_scale_bitshift1) != self.num_channels:
                raise ValueError("number of elements in rate scale bit-shift vector should be the same as number of channels in input signal!")

            self.spike_rate_scale_bitshift1 : P_array = Parameter(np.asarray(spike_rate_scale_bitshift1, dtype=np.int64))
        ...
        if isinstance(spike_rate_scale_bitshift2, Number):
            ...
            # the same EPS for all the channels
            self.spike_rate_scale_bitshift2: P_array = Parameter( (spike_rate_scale_bitshift2 * np.ones(self.num_channels)).astype(np.int64) )
            
        else:
            if len(spike_rate_scale_bitshift2) != self.num_channels:
                raise ValueError("number of elements in rate scale bit-shift vector should be the same as number of channels in input signal!")

            self.spike_rate_scale_bitshift2 : P_array = Parameter(np.asarray(spike_rate_scale_bitshift2, dtype=np.int64))
        ...
        if np.any(self.spike_rate_scale_bitshift1 <= self.spike_rate_scale_bitshift2):
            raise ValueError(
                """
                For smooth spike rate variation, we use `M(t)<<b1 - M(t)<<b2` as the spike generation threshold. 
                This requires than b1 in all channels is larger than b2 so that the threshold is always positive.
                """
            )
        ...
        if isinstance(EPS_vec, Number):
            if not isinstance(EPS_vec, int):
                raise ValueError("EPS (lower bound on threshold in DN) should be an integer!")
            
            # the same EPS for all the channels
            self.EPS_vec: P_array = Parameter( (EPS_vec * np.ones(self.num_channels)).astype(np.int64) )
        else:
            if len(EPS_vec) != self.num_channels:
                raise ValueError("number of EPS should be the same as number of channels in input signal!")
        ...
        if isinstance(fixed_threshold_vec, Number):
            if not isinstance(fixed_threshold_vec, int):
                raise ValueError("fixed thresholds used for spike generation in non-DN mode should be all integer!")
            
            self.fixed_threshold_vec: P_array = Parameter( (fixed_threshold_vec * np.ones(self.num_channels)).astype(np.int64) )
        else:
            if len(fixed_threshold_vec) != self.num_channels:
                raise ValueError("The number of thresholds should be the same as the number of channels!")
            
            self.fixed_threshold_vec: P_array = Parameter(np.asarray(fixed_threshold_vec, dtype=np.int64).ravel())            
    
        if isinstance(low_pass_bitshift, Number):
            if not isinstance(low_pass_bitshift, int):
                raise ValueError("number of bit-shifts used in low-pass filtering should be an integer!")
            
            self.low_pass_bitshift: P_array = Parameter( (low_pass_bitshift * np.ones(self.num_channels)).astype(np.int64) )
        else:
            self.low_pass_bitshift : P_array = Parameter(np.asarray(low_pass_bitshift, dtype=np.int64))
    ...
    @type_check
    def evolve(self, sig_in: np.ndarray, mode_vec: Union[int, np.ndarray]=1, joint_normalization: bool = False, record: bool=False):
        ...
        Args:
            ...
            mode_vec (Union[int, np.ndarray], optional): C-dim mode vector: 
                ...
                Defaults to None.
                Note: one may decide to switch mode from time-to-time if needed by passing a vector
                of mode of the same dimension of `sig_in`.
                record (bool): record the states during spike generation. Defaults to False.
            joint_normalization (bool): if normalization happens separately or jointly across all channels. Defaults to True.
        ...
        if isinstance(mode_vec, Number):
            if mode_vec not in {0,1}:
                raise ValueError("mode can have only values {0,1}: 0 => fixed-threshold mode, 1 => divisive normalization mode")

            mode_vec = (mode_vec * np.ones(num_channels)).astype(np.int64)
            
        else:
            if len(set(np.unique(mode_vec)) - {0,1}) != 0:
                raise ValueError("mode can have only values {0,1}: 0 => fixed-threshold mode, 1 => divisive normalization mode")
            
            mode_vec = np.asarray(mode_vec, dtype=np.int64)
    ...
    def __repr__(self):
        string = 'Synchronous divisive normalization module\n' +\
            '-'*41+'\n'+\
...
try:
    ...
    def jax_spike_gen(sig_in: np.ndarray, mode_vec:np.ndarray, spike_rate_scale_bitshift1:np.ndarray, spike_rate_scale_bitshift2:np.ndarray, low_pass_bitshift:np.ndarray, EPS_vec:np.ndarray, fixed_threshold_vec:np.ndarray, joint_normalization:bool = False, record:bool=False)->Tuple[np.ndarray, Dict[str,np.ndarray]]:
        ...
        def _compiled_spike_gen(sig_in:jnp.ndarray, record:bool=False):
            ...
            def forward(state_in_tuple: jnp.ndarray, data_in_tuple:List[jnp.ndarray])->Tuple[jnp.ndarray, List[jnp.ndarray]]:
                ...
                state_high_res_filter = state_high_res_filter - state_low_res_filter + sig_in_rect
                
                ### set the threshold for DN module
                adpative_thresholds = state_low_res_filter
                
                # put a lower bound on the thresholds using EPS
                adpative_thresholds = (jnp.abs(adpative_thresholds - EPS_vec) + adpative_thresholds + EPS_vec) >> 1
                ...
                adpative_thresholds = (adpative_thresholds<<spike_rate_scale_bitshift1) - (adpative_thresholds<<spike_rate_scale_bitshift2)
                
                # decide on separate or joint scaling of the spike rate across channels
                if joint_normalization:
                    adpative_thresholds = jnp.max(adpative_thresholds) * jnp.ones_like(adpative_thresholds)
                
                # decide on the final threshold: adaptive via DN or fixed thresholds
                applied_thresholds = (1-mode_vec) * fixed_threshold_vec + mode_vec * adpative_thresholds
                ...
                # copy the states
                state_out_tuple = (state_high_res_filter, state_low_res_filter, state_IAF)

                # copy the output
                # NOTE: here we have the option to include all the states we are interested in the output
                if record:
                    # return all the simulation states
                    data_out_tuple = (spikes, state_IAF, applied_thresholds, state_high_res_filter, state_low_res_filter)
                else:
                    # return only spikes
            ...
            initial_state = (jnp.zeros(num_channels, dtype=jnp.int64), jnp.zeros(num_channels, dtype=jnp.int64), jnp.zeros(num_channels, dtype=jnp.int64))
            ...
            if record:
                final_state_tuple, (spikes, state_IAF, adpative_thresholds, state_high_res_filter, state_low_res_filter) = jax.lax.scan(forward, initial_state, data_in_tuple)
                return spikes, (state_IAF, adpative_thresholds, state_high_res_filter, state_low_res_filter)
            else:
                final_state_tuple, (spikes,) = jax.lax.scan(forward, initial_state, data_in_tuple)
                return spikes
        
        if record:
            spikes, (state_IAF, applied_thresholds, state_high_res_filter, state_low_res_filter) = _compiled_spike_gen(
                sig_in=sig_in, 
                record=record
            )
    ...
    def fjax_spike_gen(sig_in: np.ndarray, mode_vec:np.ndarray, spike_rate_scale_bitshift1:np.ndarray, spike_rate_scale_bitshift2:np.ndarray, low_pass_bitshift:np.ndarray, EPS_vec:np.ndarray, fixed_threshold_vec:np.ndarray, joint_normalization:bool=False, record:bool=False)->Tuple[np.ndarray, Dict[str,np.ndarray]]:
        """this module processes the input signal in various channels and converts the into spikes via joint divisive normalization.
        ...
        def _compiled_spike_gen(sig_in:jnp.ndarray, record:bool=False):
            ...
            def forward(state_in_tuple: jnp.ndarray, data_in_tuple:List[jnp.ndarray])->Tuple[jnp.ndarray, List[jnp.ndarray]]:
                ...
                state_low_res_filter = jnp.fix(state_high_res_filter/ (2**low_pass_bitshift))
                state_high_res_filter = state_high_res_filter - state_low_res_filter + sig_in_rect
                ...
                # put a lower bound on the thresholds using EPS
                adpative_threshold_vec = jnp.round( (jnp.abs(adpative_thresholds - EPS_vec) + adpative_thresholds + EPS_vec) / 2.0)

                # scale the thresholds based on the target spike rate scaling parameter
                adpative_threshold_vec = adpative_threshold_vec * (2**spike_rate_scale_bitshift1 - 2**spike_rate_scale_bitshift2)
                
                # decide on separate of joint spike normalization across all channels
                if joint_normalization:
                    adpative_threshold_vec = jnp.max(adpative_threshold_vec) * jnp.ones_like(adpative_threshold_vec)
                
                # decide on the final threshold: adaptive via DN or fixed thresholds
                applied_thresholds = (1-mode_vec) * fixed_threshold_vec + mode_vec * adpative_threshold_vec
            ...
            # use forward state update to simulate DN and spike generation
            initial_state = (jnp.zeros(num_channels, dtype=jnp.float32), jnp.zeros(num_channels, dtype=jnp.float32), jnp.zeros(num_channels, dtype=jnp.float32))
            sig_in_rect = jnp.abs(sig_in)
...
def py_spike_gen(sig_in: np.ndarray, mode_vec:np.ndarray, spike_rate_scale_bitshift1:np.ndarray, spike_rate_scale_bitshift2:np.ndarray, low_pass_bitshift:np.ndarray, EPS_vec:np.ndarray, fixed_threshold_vec:np.ndarray, joint_normalization: bool=False, record:bool=False)->Tuple[np.ndarray, Dict[str, np.ndarray]]:
    """this module processes the input signal in various channels and converts the into spikes via joint divisive normalization.
    ...
    for sig_rect_sample in sig_in_rect:
        ...
        state_high_res_filter = state_high_res_filter - (state_high_res_filter >> low_pass_bitshift) + sig_rect_sample 
        
        # compute the thresholds that need to be applied            
        adaptive_threshold_vec[:] = state_low_res_filter[:]
        adaptive_threshold_vec[adaptive_threshold_vec < EPS_vec] = EPS_vec[adaptive_threshold_vec < EPS_vec]
        adaptive_threshold_vec = (adaptive_threshold_vec << spike_rate_scale_bitshift1) - (adaptive_threshold_vec << spike_rate_scale_bitshift2) 
        
        # decide on separate (per-channel) or joint spike rate normalization across channels
        if joint_normalization:
            adaptive_threshold_vec = np.max(adaptive_threshold_vec) * np.ones_like(adaptive_threshold_vec)
        
        # choose the threshold value based on the operation `mode`
        #       mode = 0 -> fixed thresholds are used in spike generations.
        #       mode = 1 -> adaptive thresholds computed by divisive normalization are used in spike generation.
        current_thresholds = mode_vec * adaptive_threshold_vec + (1-mode_vec) * fixed_threshold_vec
    ...
    if record:
        ...
        state_high_res_filter_list = np.asarray(state_high_res_filter_list, dtype=np.int64).squeeze()
        state_low_res_filter_list = np.asarray(state_low_res_filter_list, dtype=np.int64).squeeze()
