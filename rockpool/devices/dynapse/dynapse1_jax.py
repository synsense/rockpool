...
class DynapSE1Jax(DynapSEAdExpLIFJax):
    ...
    def w_rec_from_config(
        ...
        """
        w_rec_from_config obtain a weight matrix using the parameters extracted from a samna configuration object
        ...
        :return: w_rec, rec_idx
            :w_rec: recurrent current based weight and connectivity matrix (3D, NrecxNrecx4)
        ...
        w_rec = DynapSE1Jax.weight_matrix_from_CAM(CAM_rec, idx_map, sim_config.Iw_base)

        if return_maps:
            return w_rec, idx_map
        else:
            return w_rec

    @staticmethod
    def weight_matrix_from_CAM(
        CAM_rec: np.ndarray,
        idx_map: Dict[int, NeuronKey],
        Iw_base: Union[Dict[Tuple[np.uint8, np.uint8], np.ndarray], np.ndarray],
    ) -> np.ndarray:
        """
        weight_matrix_from_CAM uses the `DynapSEAdExpLIFJax` weight matrix method to obtain a weight matrix from the recurrent CAM matrix.
        ...
        :param CAM_rec: recurrent CAM matrix, the number of CAMs defined per connection per neuron (3D, NrecxNrecx4)
        :type CAM_rec: np.ndarray
        :param idx_map: a dictionary of the mapping between matrix indexes of the neurons and their neuron keys
        :type idx_map: Dict[int, NeuronKey]
        :param Iw_base: base weight matrix dictionary. In general, 4 base weights are possible.
        :type Iw: Union[Dict[Tuple[np.uint8, np.uint8], np.ndarray], np.ndarray]
        ...
        shape = CAM_rec.shape
        CAM_rec = CAM_rec.astype(int)
        w_rec = onp.zeros(shape, dtype=np.float32)
        cores = Router.core_matrix_from_idx_map(idx_map)

        # There can be multiple CAMs defined
        while onp.sum(CAM_rec) > 0:
            # [] TODO : For now, w_recs are added up linearly. It has to be non-linear!
            bit_masks = CAM_rec.astype(bool) * DynapSE1Jax.bit_mask
            w_rec += DynapSEAdExpLIFJax.weight_matrix(Iw_base, cores, bit_masks)
            CAM_rec = onp.clip(CAM_rec - 1, 0, None)
