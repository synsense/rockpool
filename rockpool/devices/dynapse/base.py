...
class DynapSE:
    ...
    @staticmethod
    def weight_matrix(
        Iw_base: Union[Dict[Tuple[np.uint8], np.ndarray], jnp.DeviceArray],
        core_vector: Union[np.ndarray, jnp.DeviceArray],
        ...
        Note that if Iw_base passed is a dictionary, then the `core_vector` should be an `np.ndarray` of `np.uint8` tuples, the keys of the dictionary
        If Iw_base passed is a `jnp.DeviceArray`, the core vector should also be `jnp.DeviceArray`

        :param Iw_base: base weight matrix dictionary, 4 base weights possible, therefore 4-bit bit-masks are defined
        :type Iw_base: Union[Dict[Tuple[np.uint8], np.ndarray], jnp.DeviceArray]
            Iw_base = {
        ...
        :type core_vector: Union[np.ndarray, jnp.DeviceArray]
        ...
        if isinstance(Iw_base, dict):
            ...
            Iw_base = jnp.vstack(jnp.array([list(Iw) for Iw in Iw_base.values()]))
            core_vector = jnp.array(
        ...
        W = jnp.sum(bits_trans * Iw_base[core_vector], axis=-1).transpose(1, 2, 0)
        return W
