...
def test_imports():
    from rockpool.devices.xylo.xylo_a3 import PDM_ADC, PDM_Microphone
...
def test_pdm_adc():
    ...
    from rockpool.devices.xylo.xylo_a3 import PDM_Microphone, PDM_ADC
    ...
    for _ in range(num_random_tests):
        ...
        sig_in = np.einsum('i, ij -> j', amp_vec, np.sin(2*np.pi*freq_vec.reshape(-1,1) * time_vec.reshape(1,-1) + phase_vec.reshape(-1,1)))

        # normalize its amplitude for PDM mic
        sig_in = safe_amplitude * sig_in / np.max(np.abs(sig_in))

        # apply PDM modulation in PDM mic
        sig_out = pdm_adc(sig_in, sample_rate=sample_rate, record=True)
        ...
        corr = np.max(np.convolve(sig_in[::-1], sig_out))/(norm(sig_in) * norm(sig_out))
