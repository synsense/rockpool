...
def test_JSVD_gravity_rotated():
    ...
    # correct the signs as well
    corr_ordered_signcorrected = corr_ordered @ np.diag(np.sign(np.diag(corr_ordered)))

    deviation = np.linalg.norm(corr_ordered_signcorrected - np.eye(3)) / np.linalg.norm(
        np.eye(3)
    )
    ...
    if deviation < max_deviation:
        print(
            f"test passed => relative deviation: {deviation} below the permitted threshold: {max_deviation}!"
        )
        num_pass += 1
    else:
        print(
            f"test did not pass => relative deviation: {deviation} above the permitted threshold: {max_deviation}!"
        )
    print("*" * 100)
