...
def test_peak_loss():
    ...
    pl = PeakLoss(max_interval=max_interval, weight_nontarget=1.0, target_output=target_output)
    tmax = 5
    ...
    for i in range(batch_size):
        vmem_max_target[i] = torch.mean(vmem[i, tmax: tmax+max_interval, targets[i]], axis=0)
    loss_target = mse(vmem_max_target, target_output * torch.ones(batch_size))
    assert torch.allclose(loss_target, pl.loss_target(vmem, targets))
    ...
    vmem_max_target = torch.ones(batch_size) * (torch.max(vmem) + 1e-5)
    for i in range(batch_size):
        vmem_max_target[i] = (2*vmem[i, -2, targets[i]] + vmem[i, -1, targets[i]]) / 3.
    loss_target = mse(vmem_max_target, target_output * torch.ones(batch_size))
    assert torch.allclose(loss_target, pl.loss_target(vmem, targets))
    ...
    for i in range(batch_size):
        vmem_max_target[i] = torch.mean(vmem[i, tmax: tmax + max_interval, targets[i]], axis=0)
    assert torch.allclose(pl.loss_nontarget(vmem, targets), pl.loss_target(vmem, targets))
...
def test_binary_peak_loss():
    ...
    # pick event time such that the loss window fits within the sample
    pl = BinaryPeakLoss(max_interval=max_interval, weight_negatives=1.0, target_output=target_output)
    tmax = 5
    ...
    for i in range(n_positives):
        vmem_max_positives[i] = torch.mean(vmem[targets == 1, tmax: tmax+max_interval, 0][i], axis=0)
    loss_positives = mse(vmem_max_positives, target_output * torch.ones(n_positives))
    assert torch.allclose(loss_positives, pl.loss_positives(vmem, targets))

    # pick event time as the second last time point and window size three, thus the output at event time should be
    # counted twice
    pl = BinaryPeakLoss(max_interval=3, weight_negatives=1.0, target_output=target_output)
    vmem = torch.rand((batch_size, T, 1))
    vmem[targets == 1, -2, 0] = torch.max(vmem) + 1e-5
    vmem_max_positives = torch.ones(n_positives) * (torch.max(vmem) + 1e-5)
    for i in range(n_positives):
        vmem_max_positives[i] = (2*vmem[targets == 1, -2, 0][i] + vmem[targets == 1, -1, 0][i]) / 3.
    loss_positives = mse(vmem_max_positives, target_output * torch.ones(n_positives))
    assert torch.allclose(loss_positives, pl.loss_positives(vmem, targets))
    ...
    # pick event time such that the loss window fits within the sample
    max_interval = T
    pl = BinaryPeakLoss(max_interval=max_interval, weight_negatives=1.0, target_output=0.0)
    tmax = 0
    ...
    for i in range(n_positives):
        vmem_max_positives[i] = torch.mean(vmem[targets == 1, tmax: tmax + max_interval, 0][i], axis=0)
    assert torch.allclose(pl.loss_negatives(vmem, targets), pl.loss_positives(vmem, targets))
