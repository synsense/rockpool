...
def test_cliaf_evolve_subtracting():
    ...
    # - Expectation: Input spike will cause neuron 0 to spike 2 times at t=0.5
    #                These spikes will cause neuron 1 to spike once at t=0.6
    #                Last input spike will not have effect because evolution
    #                stops beforehand
    print(tsOutput.vtTimeTrace)
    assert np.allclose(tsOutput.vtTimeTrace, np.array([0.5, 0.5, 0.6])), \
...
def test_cliaf_evolve_resetting():
    ...
    # - Expectation: Input spike will cause neuron 0 to spike once at t=0.5
    ...
    assert np.allclose(tsOutput.vtTimeTrace, np.array([0.5])), \
    "Output spike times not as expected"
